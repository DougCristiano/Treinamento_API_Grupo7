...
async function buscar() {
  ...
  let lobosnaoadotados = lobodalistabusca.filter(
    (lobo) => lobo.adotado === false
  );

  if (opcaoAtivada === true) {
    loboescolhido = lobosadotados.find(
      (item) => item.nome.toLowerCase() == nomeminusculo
    );
  } else {
    loboescolhido = lobosnaoadotados.find(
      (item) => item.nome.toLowerCase() == nomeminusculo
    );
  }
  ...
  if (loboescolhido === undefined) {
    if (opcaoAtivada === true) {
      alert(
        `O lobo ${pesquisa} não está no banco de dados ou ele não foi adotado ainda`
      );
    } else {
      alert(
        `O lobo ${pesquisa} não está no banco de dados ou ele já foi adotado `
      );
    }
    ...
    let foto = document.createElement("img");
    let src = loboescolhido.imagem;
    if (
      src != "../images/loboexemplo2.png" &&
      src != "../images/loboexemplo.png"
    ) {
      if (Number(loboescolhido.id) % 2 === 0) {
        src = "../images/loboexemplo2.png";
      } else {
        src = "../images/loboexemplo.png";
      }
